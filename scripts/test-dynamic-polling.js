...
async function testDynamicPolling() {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (wasCatchingUp !== isCatchingUp) {
            ...
            log.info(
                `🔄 Polling interval changed: ${currentInterval}ms → ${newInterval}ms (${isCatchingUp ? "catching up" : "caught up"})`,
            );
            currentInterval = newInterval;
        }

        log.info(
            `📊 Iteration ${i + 1}: latest=${latestBlock}, from=${fromBlock}, catchingUp=${isCatchingUp}, interval=${currentInterval}ms`,
        );

        if (fromBlock <= latestBlock) {
            // Simulate indexing
            await mockEventIndexer.indexEvents(
                fromBlock,
                Math.min(fromBlock + 999, latestBlock),
            );
