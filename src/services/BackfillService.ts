...
export class BackfillService {
    ...
    async backfillEvents(fromBlock: number, toBlock: number): Promise<void> {
        ...
        log.info(
            `📦 Processing in ${totalChunks} chunks of ${this.chunkSize} blocks each`,
        );
        ...
        for (let chunk = 0; chunk < totalChunks; chunk++) {
            ...
            log.info(
                `🔄 Processing chunk ${chunk + 1}/${totalChunks}: blocks ${chunkStart}-${chunkEnd}`,
            );
            ...
            try {
                ...
                log.info(
                    `✅ Chunk ${chunk + 1} completed. Progress: ${progress}% (${processedBlocks}/${totalBlocks} blocks)`,
                );
                ...
                log.error(
                    `❌ Error processing chunk ${chunk + 1}:`,
    ...
    async findContractDeploymentBlock(): Promise<number> {
        log.info(
            `🎯 Using known deployment block: ${ROULETTE_DEPLOYMENT_BLOCK}`,
        );
    ...
    async backfillFromDeployment(): Promise<void> {
        ...
        log.info(
            `🚀 Starting backfill from deployment block ${deploymentBlock} to latest block ${latestBlock}`,
        );
    ...
    async backfillRecentBlocks(blocksBack: number = 1000): Promise<void> {
        ...
        log.info(
            `🚀 Starting backfill of recent ${blocksBack} blocks: ${fromBlock} to ${latestBlock}`,
        );
